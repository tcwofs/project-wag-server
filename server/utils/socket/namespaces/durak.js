...
module.exports = io => {
  ...
  durak.on('connection', (socket, callback) => {
    ...
    const updateOrder = room => {
      ...
      if (room.users[first].hand.length === 0) first = first + 1;
      if (room.users[firs + 1].hamd.length === 0) first = first + 2;

      room.users = giveStatus(room.users, first);
      updatedField(room);
    };
    ...
    socket.on('attack', ({ card, roomname, second }) => {
      ...
      if (room.users.filter(user => user.status === 'defending' && user.hand.length === 0).length !== 0) return;
      if (userindex !== -1) {
          ...
          if (!!room.deck.field.length) {
            for (let i = 0; i < room.deck.field.length; i++) {
              if (room.deck.field[i].length === 1) {
                ...
                if ((cardChar === fieldChar && cardNum > fieldNum) || cardChar === room.deck.lastcard[0].match(/[SHDC]/g).toString()) {
