...
const addUserToRoom = ({ room, password, user }) => {
  ...
  const existingRoom = chatRooms.find((item) => item.roomname === room.roomname);
  ...
  if (room && existingUser === -1) {
    if (!passwordRoom || (passwordRoom.password && passwordRoom.password === password)) {
      ...
      room.messages.push({ id: uuidv4(), from: 'System', username: 'System', text: `${user.username} connected` });
      return 0;
    } else {
  ...
  if (roomIndex !== -1) {
    ...
    if (userIndex !== -1) {
      chatRooms[roomIndex].messages.push({
        ...
        from: 'System',
        username: 'System',
        text: `${chatRooms[roomIndex].users[userIndex].username} left`,
      });
...
const removeUserFromAllRoom = ({ user }) => {
  ...
  if (indexes.length !== 0) {
    for (let i = indexes.length - 1; i >= 0; i--) {
      chatRooms[indexes[i].room].messages.push({
        id: uuidv4(),
        from: 'System',
        username: 'System',
        text: `${chatRooms[indexes[i].room].users[indexes[i].user].username} left`,
      });
...
const pushMessage = ({ message, roomname, user }) => {
  let roomIndex = chatRooms.findIndex((room) => room.roomname === roomname);
  if (roomIndex !== -1) chatRooms[roomIndex].messages.push({ id: uuidv4(), from: `${user.id}`, username: `${user.username}`, text: message });
  return 0;
};
